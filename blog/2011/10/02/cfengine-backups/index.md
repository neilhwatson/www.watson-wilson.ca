...
Cfengine is flexible enough to allow for advanced time scheduling (see Cfengine
...
shell commands allow you to use your own backup tools.
...
Here we set some variables for a backup directory and a list of things to
...
list variables must be fully scoped by listing their bundle name.

The bundle is called at 0100 hours. If_elapsed ensures that the backup in only
promised once during that hour. Depends_on shows what other bundles might need
to be done first. In this case I need a Subversion promise ahead of time.

The magic here is that not only will the bundle go off at 0100 but like all
Cfengine promises it will continually try, for every run during that hour, to
ensure that that promise is kept. Now let’s look at the called bundle.
...
The bundle is quite simple. A tar command is run using the passed variables as
...
necessary in Nova since it keeps track of promise compliance automatically.
...
I have a directory of images from my camera work. I like to back this up to
another hard drive on my workstation. First the method.
...
This method passes two strings, both directories, to a bundle. The if_elapsed
body sets the frequency of this backup to once per week. Now the bundle.
...
Two files promises are found here. The first ensure that the destination
...
Some of my backups are sent to an off-site host. I don't fully trust the
...
The first method calls a bundle make encrypted copies of my backup tar balls.
...
This bundle calls GNUpg via a commands promise to encrypt the given tar file. A
...
This last bundle is another files promise that performs a copy. The file_select
...
Should you be using Nova you can use something called remote classes. Remote
...
it is set then kickoff of the remote backup bundle.
